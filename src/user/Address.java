...
public class Address implements IAddress{
	...
	User user;
	private final String[] addresses = {"Nadejda", "Lyulin", "Krasna Poliyana", "Centyr", "Obelya", "Geo Milev"}; 
	private final String[] streets = {"Bratin Dol", "Alabin", "Vitosha", "Bylgariya", "Rakovska", "Cherkovna"}; 
	private final String[] nameOFAddress = {"home", "other"};
	//private String city;
	...
	public Address(User user) {
		...
		addAddress();
		addToDB(getStreet(), getBuilding(), getAddressName());
...
//	}
	...
	public boolean setNeighbourhood() {
		...
		String neighbourhoodInput = null;
		if(sc.hasNextLine()) {
			neighbourhoodInput = sc.nextLine();
		} else {
			neighbourhoodInput = addresses[new Random().nextInt(addresses.length)];
		}
	...
	public boolean setStreet() {
		...
		String streetInput = null;
		if(sc.hasNextLine()) {
			streetInput = sc.nextLine();
		} else {
			streetInput = streets[new Random().nextInt(streets.length)];;
		}
...
//	}
	...
	public boolean setBuilding() {
		...
		Integer buildingInput = new Random().nextInt(50)+1;
		String temp = buildingInput.toString();
		if(!temp.equals("") && temp.matches("^[0-9]{1,7}$")) {
			try {
...
//	}
	...
	public void changeAddress(){
		...
		int currentBuilding = this.building;
...
//		while(!setTelephoneNumber());
	...
	public void setAddressName() {
		boolean check = false;
		//System.out.println("Please add address name:");
		while(!check) {
			String address = nameOFAddress[new Random().nextInt(nameOFAddress.length)];
			if(!address.equals("")) {
				try {
	...
	private void addToDB(String street,int number,String name) {
		int userID = new Random().nextInt(10)+1;
		int neighID = new Random().nextInt(3)+1;
		String query = ("INSERT into addresses (street, building_number, address_name, neighborhood_ID, users_ID) values (?,?,?,?,?)");
